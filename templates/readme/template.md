...
- **Mode**: {{ readme_mode }}
{% if existing_readme_path %}- **Existing README**: {{ existing_readme_path }}{% endif %}
- **Sections**: {{ sections | join(', ') }}
- **Include Badges**: {{ include_badges }}
- **Include TOC**: {{ include_toc }}
- **Code Examples Count**: {{ code_examples_count }}
...
5. {% if project_name %}Use provided project name: {{ project_name }}{% else %}Derive project name from repository or package config{% endif %}
...
{% if readme_mode == 'improve' or readme_mode == 'audit' %}
## Step 3b: Existing README Analysis
1. Read existing README at `{{ existing_readme_path if existing_readme_path else 'README.md' }}`
2. Identify accurate vs outdated information
3. Find missing sections compared to requested: {{ sections | join(', ') }}
4. Note inconsistencies with current codebase
5. List improvement opportunities
{% endif %}

## Step 4: Code Example Extraction (15-20% of time)
1. Find {{ code_examples_count }} representative usage examples
...
You MUST produce a README with these sections (include only requested sections from {{ sections | join(', ') }}):
...
### Basic Examples
Provide {{ code_examples_count }} code examples showing common use cases:
...
|---------|--------|--------|
| [Section] | ✓ Accurate / ⚠️ Outdated / ✗ Incorrect | Description |
...
2. [Priority] Improvement description

### Outdated Content
List specific outdated items with corrections needed.
{% endif %}
...
{
  "project_name": "string",
  "project_type": "library|cli|webapp|api|other",
  "languages": ["string"],
  "sections": {
    "overview": {
      "description": "string",
    ...
    "installation": {
      "prerequisites": [{"name": "string", "version": "string"}],
      "install_commands": [{"method": "string", "command": "string"}]
    },
    "usage": {
      "quick_start": "string",
      "examples": [{"title": "string", "code": "string", "description": "string"}],
      "configuration": [{"name": "string", "description": "string", "default": "string"}]
    },
    "api": {
      "functions": [{"name": "string", "description": "string", "params": ["string"], "returns": "string"}],
      "classes": [{"name": "string", "description": "string"}],
      "cli_commands": [{"command": "string", "description": "string", "options": ["string"]}]
    },
    "contributing": {
      "setup": "string",
      "test_command": "string",
  ...
  "audit_results": {
    "accuracy": [{"section": "string", "status": "accurate|outdated|incorrect", "issues": "string"}],
    "missing": ["string"],
